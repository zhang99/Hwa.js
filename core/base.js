...
{
    //hwaui 继承方法
    Function.prototype.hwaExtend = function (parent, overrides)
    {
    ...
    //延时加载
    Function.prototype.hwaDefer = function (o, defer, args)
    {
        var fn = this;
    ...
    window.hwa = $.hwaui = {
        ...
        remove: function (arg)
        {
            ...
            {
                delete hwa.managers[arg];
            }
            else if (typeof arg == "object")
            {
                if (arg instanceof hwa.core.Component)
                {
                    delete hwa.managers[arg.id];
                }
                else
                {
                    if (!$(arg).attr(this.idAttrName)) return false;
                    delete hwa.managers[$(arg).attr(this.idAttrName)];
                }
            }
        },
        //获取hwaui对象
        ...
        {
            idAttrName = idAttrName || "hwauiid";
            if (typeof arg == "string" || typeof arg == "number")
            {
                return hwa.managers[arg];
            }
            else if (typeof arg == "object")
            {
        ...
        //$.fn.hwa{Plugin} 和 $.fn.hwaGet{Plugin}Manager
        //会调用这个方法,并传入作用域(this)
        //parm [plugin]  插件名
        //parm [args] 参数(数组)
        ...
        run: function (plugin, args, ext)
        {
            if (!plugin) return;
            ext = $.extend({
                defaultsNamespace: 'hwaDefaults',
                methodsNamespace: 'hwaMethods',
            ...
            plugin = plugin.replace(/^hwaGet/, '');
            plugin = plugin.replace(/^hwa/, '');
            if (this == null || this == window || ext.isStatic)
            {
                if (!hwa.plugins[plugin])
                {
                    hwa.plugins[plugin] = {
                        fn: $[hwa.pluginPrev + plugin],
                        isStatic: true
                    };
                }
                return new $.hwaui[ext.controlNamespace][plugin]($.extend({}, $[ext.defaultsNamespace][plugin] || {}, $[ext.defaultsNamespace][plugin + 'String'] || {}, args.length > 0 ? args[0] : {}));
            }
            if (!hwa.plugins[plugin])
            {
                hwa.plugins[plugin] = {
                    fn: $.fn[hwa.pluginPrev + plugin],
                    isStatic: false
                };
            }
            if (/Manager$/.test(plugin)) return hwa.get(this, ext.idAttrName);
            this.each(function ()
            {
                if (this[ext.idAttrName] || $(this).attr(ext.idAttrName))
                {
                    var manager = hwa.get(this[ext.idAttrName] || $(this).attr(ext.idAttrName));
                ...
                if (ext.hasElement)
                {
                    new $.hwaui[ext.controlNamespace][plugin](this, p);
                }
                else
                {
                    new $.hwaui[ext.controlNamespace][plugin](p);
                }
            });
            if (this.length == 0) return null;
            if (args.length == 0) return hwa.get(this, ext.idAttrName);
            if (typeof args[0] == 'object') return hwa.get(this, ext.idAttrName);
            if (typeof args[0] == 'string')
            {
                var manager = hwa.get(this, ext.idAttrName);
    ...
    //关联起来
    hwa.defaults = $.hwaDefaults;
    hwa.methods = $.hwaMethos;

    //获取hwaui对象
    //parm [plugin]  插件名,可为空
    $.fn.hwa = function (plugin)
    {
        if (plugin)
        {
            return hwa.run.call(this, plugin, arguments);
        }
        else
        {
            return hwa.get(this);
    ...
    hwa.core.Component = function (options)
    {
        //事件容器
    ...
    $.extend(hwa.core.Component.prototype, {
        __getType: function ()
        {
            return 'hwa.core.Component';
        },
        __idPrev: function ()
        {
            return 'hwaui';
    ...
    //parm [options] 组件的参数
    hwa.core.UIComponent = function (element, options)
    {
        hwa.core.UIComponent.base.constructor.call(this, options);
    ...
    hwa.core.UIComponent.hwaExtend(hwa.core.Component, {
        __getType: function ()
        {
            return 'hwa.core.UIComponent';
        },
        //扩展方法
        ...
        _init: function ()
        {
            ...
            if (!this.element)
            {
                this.id = this.options.id || hwa.getId(this.__idPrev());
            }
            else
            {
                this.id = this.options.id || this.element.id || hwa.getId(this.__idPrev());
            }
            //存入管理器池
            hwa.add(this);
            ...
            //读取hwaui这个属性，并加载到参数，比如 hwaui = "width:120,heigth:100"
            var p = this.options;
            if ($(this.element).attr("hwaui"))
            {
                try
                {
                    var attroptions = $(this.element).attr("hwaui");
            ...
            { 
                ...
                var defaultOp = hwa.defaults[control]; 
                if (!defaultOp) return op;
                for (var name in defaultOp)
                {
                    if (jelement.attr(hwa.attrPrev + "-" + name))
                    {
                        var value = jelement.attr(hwa.attrPrev + "-" + name);
        ...
        _rendered: function ()
        {
            if (this.element)
            {
                $(this.element).attr("hwauiid", this.id);
            }
        },
    ...
    //表单控件基类
    hwa.controls.Input = function (element, options)
    {
        hwa.controls.Input.base.constructor.call(this, element, options);
    };

    hwa.controls.Input.hwaExtend(hwa.core.UIComponent, {
        __getType: function ()
        {
            return 'hwa.controls.Input';
        },
        attr: function ()
        {
            return ['nullText'];
    ...
    hwa.win = {
        ...
        mask: function (win)
        {
            function setHeight()
            {
                if (!hwa.win.windowMask) return;
                var h = $(window).height() + $(window).scrollTop();
                hwa.win.windowMask.height(h);
            }
            if (!this.windowMask)
            {
                this.windowMask = $("<div class='l-window-mask' style='display: block;'></div>").appendTo('body');
                $(window).bind('resize.hwauiwin', setHeight);
        ...
        unmask: function (win)
        {
            ...
            {
                var winid = jwins.eq(i).attr("hwauiid");
                if (win && win.id == winid) continue;
                //获取hwaui对象
                var winmanager = hwa.get(winid);
        ...
        {
            var wins = hwa.find(hwa.core.Win);
            for (var i in wins)
            {
                var w = wins[i];
                if (w == win)
    ...
    //窗口基类 window、dialog
    hwa.core.Win = function (element, options)
    {
        hwa.core.Win.base.constructor.call(this, element, options);
    };

    hwa.core.Win.hwaExtend(hwa.core.UIComponent, {
        __getType: function ()
        {
            return 'hwa.controls.Win';
        },
        mask: function ()
        {
    ...
    hwa.toJSON = typeof JSON === 'object' && JSON.stringify ? JSON.stringify : function (o)
    {
        var f = function (n)
        {
            return n < 10 ? '0' + n : n;
        ...
        if (type === 'object')
        {
            ...
            if (o.constructor === Array)
            {
                ...
                {
                    pairs.push(hwa.toJSON(o[i]) || 'null');
                }
                return '[' + pairs.join(',') + ']';
            }
            var name, val;
    ...
    hwa.getEditor = function (e)
    {
            ...
            create: function (container, editParm, controlOptions)
            {
                ...
                if (editParm.field)
                {
                    ...
                    if (!e.body)
                    {
                        ...
                        {
                            inputBody.attr('validate', hwa.toJSON(field.validate));
                        }
                    }
                ...
                //返回的是hwaui对象
                var lobj = inputBody['hwa' + control](p);
                if (isInGrid)
                {
        ...
        return $.extend({}, defaultOp, hwa.editorCreatorDefaults || {}, e);
    }
    //几个默认的编辑器构造函数
    hwa.editors = {
        "text": {
    ...
    hwa.editors["string"] = hwa.editors["text"];
    hwa.editors["select"] = hwa.editors["combobox"];
    hwa.editors["int"] = hwa.editors["digits"];
    hwa.editors["float"] = hwa.editors["number"];
    hwa.editors["chk"] = hwa.editors["checkbox"];
    hwa.editors["popupedit"] = hwa.editors["popup"];

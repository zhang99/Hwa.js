...
(function ($)
{
    ...
    $.hwaDialog = function ()
    {
        return l.run.call(null, "hwaDialog", arguments, { isStatic: true });
    };

    //dialog 图片文件夹的路径 预加载
    $.hwaui.DialogImagePath = "../../lib/hwaUI/skins/Aqua/images/win/";
    ...
    $.hwaMethos.Dialog = $.hwaMethos.Dialog || {};
    ...
    l.controls.Dialog.hwaExtend(l.core.Win, {
        __getType: function ()
        {
            return 'Dialog';
        },
        ...
        _render: function ()
        {
            ...
            else if (p.url)
            {
                ...
                else
                {
                    ...
                    var framename = p.name ? p.name : "hwawindow" + new Date().getTime();
                    ...
                    // 加入以下代码 
                    tmpId = 'jquery_hwaui_' + new Date().getTime();
        ...
        show: function ()
        {
            ...
            //前端显示 
            $.hwaui.win.setFront.hwaDefer($.hwaui.win, 100, [g]);
        },
        setUrl: function (url)
        {
            this._setUrl(url);
            ...
            if ($.fn.hwaDrag)
            {
                g.draggable = g.dialog.hwaDrag({
                    ...
                    onStopDrag: function ()
                    {
                        ...
                        if (p.target)
                        {
                            var triggers1 = l.find($.hwaui.controls.DateEditor);
                            var triggers2 = l.find($.hwaui.controls.ComboBox);
                            //更新所有下拉选择框的位置
        ...
        _onReisze: function ()
        {
            ...
            if (p.target || p.url)
            {
                var manager = $(p.target).hwa();
                if (!manager) manager = $(p.target).find(":first").hwa();
                if (!manager) return;
        ...
        _applyResize: function ()
        {
            ...
            if ($.fn.hwaResizable)
            {
                g.resizable = g.dialog.hwaResizable({
    ...
    $.hwaDialog.open = function (p)
    {
        return $.hwaDialog(p);
    };
    $.hwaDialog.close = function ()
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        for (var i in dialogs)
        {
            var d = dialogs[i];
            d.destroy.hwaDefer(d, 5);
        }
        l.win.unmask();
    };
    $.hwaDialog.show = function (p)
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        if (dialogs.length)
        {
    ...
    $.hwaDialog.tip = function (options)
    {
        options = $.extend({
            showType: 'slide',
            width: 240,
        ...
        return $.hwaDialog.open(options);
    };
    $.hwaDialog.alert = function (content, title, type, callback, options)
    {
        content = content || "";
        ...
        p = {
            content: content,
            buttons: [{ text: $.hwaDefaults.DialogString.ok, onclick: btnclick }]
        };
    ...
    $.hwaDialog.confirm = function (content, title, callback)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            ...
            buttons: [
                {
                    text: $.hwaDefaults.DialogString.yes, onclick: btnclick, type: 'ok', cls: 'l-dialog-btn-ok'
                }, {
                    text: $.hwaDefaults.DialogString.no, onclick: btnclick, type: 'no', cls: 'l-dialog-btn-no'
                }
            ]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
    ...
    $.hwaDialog.warning = function (content, title, callback, options)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.hwaDefaults.DialogString.yes, onclick: btnclick, type: 'yes' }, { text: $.hwaDefaults.DialogString.no, onclick: btnclick, type: 'no' }, { text: $.hwaDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.hwaDialog.waitting = function (title)
    {
        title = title || $.hwaDefaults.Dialog.waittingMessage;
        return $.hwaDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding:4px">' + title + '</div>', allowClose: false });
    };
    $.hwaDialog.closeWaitting = function ()
    {
    ...
    $.hwaDialog.success = function (content, title, onBtnClick, options)
    {
        return $.hwaDialog.alert(content, title, 'success', onBtnClick, options);
    };
    $.hwaDialog.error = function (content, title, onBtnClick, options)
    {
        return $.hwaDialog.alert(content, title, 'error', onBtnClick, options);
    };
    $.hwaDialog.warn = function (content, title, onBtnClick, options)
    {
        return $.hwaDialog.alert(content, title, 'warn', onBtnClick, options);
    };
    $.hwaDialog.question = function (content, title, options)
    {
        return $.hwaDialog.alert(content, title, 'question', null, options);
    };


    $.hwaDialog.prompt = function (title, value, multi, callback)
    {
        ...
        p = {
            ...
            target: target,
            width: 320,
            buttons: [{ text: $.hwaDefaults.DialogString.ok, onclick: btnclick, type: 'yes' }, { text: $.hwaDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        return $.hwaDialog(p);
    };
